 ...
 *	导出
 */
public class AggCaigoudingd_zglMasterVOExcelOutputProcessor extends AbstractExcelOutputProcessor {
	...
	public Object[] getObjectValueByPks(String[] pks) throws BusinessException {
		...
		AggCaigoudingd_zglMasterVO[] queryResult = getService().listAggCaigoudingd_zglMasterVOByPk(pks);
	...
	class DefRefPropertyProcess implements IRefPropertyProcess{
		...
		public AbstractRefModel getRefModelByProperty(FormPropertyVO property) {
			...
			// 1、判断是否有树形上级参照自身字段
			if("4dc301f5-91fe-4897-968e-df1f1e30cd23".equals(classid) && pid != null && pid.equals(property.getCode())) {
